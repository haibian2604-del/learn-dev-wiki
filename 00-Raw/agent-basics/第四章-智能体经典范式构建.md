...
source: "https://github.com/datawhalechina/hello-agents/blob/main/docs/chapter4/%E7%AC%AC%E5%9B%9B%E7%AB%A0%20%E6%99%BA%E8%83%BD%E4%BD%93%E7%BB%8F%E5%85%B8%E8%8C%83%E5%BC%8F%E6%9E%84%E5%BB%BA.md"
author:
published:
created: 2026-07-31
...
3. 如果实在不知道如何获取，可以参考 [环境配置](https://github.com/datawhalechina/hello-agents/blob/main/Extra-Chapter/Extra07-%E7%8E%AF%E5%A2%83%E9%85%8D%E7%BD%AE.md)
```
# .env file
...
我们可以将这个过程形式化地表达出来，如图4.1所示。具体来说，在每个时间步 $t$ ，智能体的策略（即大语言模型 $π$ ）会根据初始问题 $q$ 和之前所有步骤的“行动-观察”历史轨迹 $\left(\left(a_{1} , o_{1}\right) , \ldots , \left(a_{t - 1} , o_{t - 1}\right)\right)$ ，来生成当前的思考 $t h_{t}$ 和行动 $a_{t}$ ：

$$
\left(t h_{t} , a_{t}\right) = \pi \left(q , \left(a_{1} , o_{1}\right) , \ldots , \left(a_{t - 1} , o_{t - 1}\right)\right)
$$

随后，环境中的工具 $T$ 会执行行动 $a_{t}$ ，并返回一个新的观察结果 $o_{t}$ ：

$$
o_{t} = T \left(a_{t}\right)
$$

这个循环不断进行，将新的 $\left(a_{t} , o_{t}\right)$ 对追加到历史中，直到模型在思考 $t h_{t}$ 中判断任务已完成。

[![ReAct范式中的“思考-行动-观察”协同循环](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/4-figures/4-1.png)](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/4-figures/4-1.png)
...
我们可以将这个两阶段过程进行形式化表达。首先，规划模型 $\pi_{\text{plan}}$ 根据原始问题 $q$ 生成一个包含 $n$ 个步骤的计划 $P = \left(p_{1} , p_{2} , \ldots , p_{n}\right)$ ：

$$
P = \pi_{\text{plan}} \left(q\right)
$$

随后，在执行阶段，执行模型 $\pi_{\text{solve}}$ 会逐一完成计划中的步骤。对于第 $i$ 个步骤，其解决方案 $s_{i}$ 的生成会同时依赖于原始问题 $q$ 、完整计划 $P$ 以及之前所有步骤的执行结果 $\left(s_{1} , \ldots , s_{i - 1}\right)$ ：

$$
s_{i} = \pi_{\text{solve}} \left(q , P , \left(s_{1} , \ldots , s_{i - 1}\right)\right)
$$

最终的答案就是最后一个步骤的执行结果 $s_{n}$ 。

[![Plan-and-Solve范式的两阶段工作流](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/4-figures/4-2.png)](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/4-figures/4-2.png)
...
如图4.3所示，这个循环可以重复进行多次，直到反思阶段不再发现新的问题，或者达到预设的迭代次数上限。我们可以将这个迭代优化的过程形式化地表达出来。假设 $O_{i}$ 是第 $i$ 次迭代产生的输出（$O\_0$ 为初始输出），反思模型 $\pi_{\text{reflect}}$ 会生成针对 $O_{i}$ 的反馈 $F_{i}$ ： $$ F\_i = \\pi\_{\\text{reflect}}(\\text{Task}, O\_i) $$ 随后，优化模型 $\pi_{\text{refine}}$ 会结合原始任务、上一版输出以及反馈，生成新一版的输出 $O_{i + 1}$ ： $$ O\_{i+1} = \\pi\_{\\text{refine}}(\\text{Task}, O\_i, F\_i) $$

[![Reflection机制中的“执行-反思-优化”迭代循环](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/4-figures/4-3.png)](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/4-figures/4-3.png)
...
这个案例充分证明，一个设计良好的 Reflection 机制，其价值不仅在于修复错误，更在于 **驱动解决方案在质量和效率上实现阶梯式的提升** ，这使其成为构建复杂、高质量智能体的关键技术之一。
...
[![](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/4-figures/4-4.png)](https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/4-figures/4-4.png)
